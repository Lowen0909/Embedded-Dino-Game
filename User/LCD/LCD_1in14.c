...
uint8_t buf[5000];
//static只限本檔案可以用
static void LCD_1IN14_SendData_DMA(const unsigned char *image,uint32_t len)
{
    uint32_t offset=0;
	LCD_1IN14_DC_1;
    LCD_1IN14_CS_0;
...
//self-defined
void LCD_partial_Clear(UWORD pos,UWORD height,UWORD WIDTH,UWORD HEIGHT)
{
//    UWORD i,j;
    LCD_1IN14_SetWindows(pos,height , pos+WIDTH-1, height+HEIGHT-1);
//    printf("\rClear\n");
	DEV_Digital_Write(DEV_DC_PIN, 1);
//	for(i = 0; i <WIDTH; i++){
//		for(j = 0; j <HEIGHT; j++){
//			DEV_SPI_WRITE((0xFFFF>>8)&0xff);
//			DEV_SPI_WRITE(0xFFFF);
//		}
//	 }
...
//self-defined
void LCD_1IN14_DrawPaint_new(const unsigned char *image,uint32_t len,UWORD xStart, UWORD yStart, UWORD xEnd,UWORD yEnd)
{
	LCD_1IN14_SetWindows(xStart,yStart,xEnd,yEnd);
	LCD_1IN14_SendData_DMA(image,len);
}
/*******************************************************************************
function:
	Setting backlight
